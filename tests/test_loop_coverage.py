...
class TestToolHint:
    def _tc(self, name, arguments):
        tc = MagicMock()
        tc.name = name
        tc.arguments = arguments
        return tc

    def test_single_tool_short_arg(self):
        tc = self._tc("web_search", {"query": "cats"})
        result = AgentLoop._tool_hint([tc])
        assert result == 'web_search("cats")'

    def test_single_tool_long_arg_truncated(self):
        long_val = "x" * 50
    ...
    def test_non_string_value_uses_name_only(self):
...
class TestCollectPluginContext:
    ...
    def test_tool_with_system_context(self):
        loop, _ = _make_loop()
        tool = MagicMock()
        tool.name = "my_tool"
    ...
    def test_tool_without_system_context(self):
        loop, _ = _make_loop()
    ...
    def test_tool_system_context_returns_empty_string(self):
        loop, _ = _make_loop()
        tool = MagicMock()
        tool.name = "t"
    ...
    def test_tool_system_context_raises(self):
        loop, _ = _make_loop()
        tool = MagicMock()
    ...
    def test_tool_system_context_returns_non_string(self):
        loop, _ = _make_loop()
        tool = MagicMock()
        tool.name = "t"
...
class TestRunAgentLoop:
    async def test_normal_response_no_tool_calls(self):
        loop, _ = _make_loop()
    ...
    async def test_error_finish_reason(self):
        loop, _ = _make_loop()
        loop.provider.chat = AsyncMock(return_value=_make_response(content="oops", finish_reason="error"))
    ...
    async def test_on_progress_called_for_tool_hint(self):
        loop, _ = _make_loop()
        ...
        async def on_progress(content, **kw):
    ...
    async def test_thinking_blocks_included(self):
        loop, _ = _make_loop()
        resp = _make_response(content="answer")
    ...
    async def test_strip_think_applied_to_content(self):
        loop, _ = _make_loop()
...
class TestProcessMessage:
    async def test_new_command_success(self):
        loop, _ = _make_loop()
    ...
    async def test_new_command_failure(self):
        loop, _ = _make_loop()
    ...
    async def test_system_channel_message(self):
        loop, _ = _make_loop()
        loop.provider.chat = AsyncMock(return_value=_make_response(content="done"))
        msg = InboundMessage(channel="system", sender_id="cron", chat_id="slack:c1", content="do thing")
        result = await loop._process_message(msg)
        assert result is not None
        assert result.channel == "slack"
        assert result.chat_id == "c1"

    async def test_system_channel_no_colon_in_chat_id(self):
        loop, _ = _make_loop()
        loop.provider.chat = AsyncMock(return_value=_make_response(content="done"))
        msg = InboundMessage(channel="system", sender_id="cron", chat_id="direct", content="do thing")
        result = await loop._process_message(msg)
        assert result is not None
        assert result.channel == "cli"

    async def test_sent_in_turn_suppresses_reply(self):
        loop, _ = _make_loop()
    ...
    async def test_no_final_content_fallback(self):
        loop, _ = _make_loop()
        loop.provider.chat = AsyncMock(return_value=_make_response(content=None, finish_reason="stop"))
        msg = InboundMessage(channel="slack", sender_id="u1", chat_id="c1", content="hello")
        result = await loop._process_message(msg)
        assert result is not None
        assert result.content is not None

    async def test_notify_tools_inbound_called(self):
        loop, _ = _make_loop()
...
class TestProcessDirect:
    ...
    async def test_returns_empty_string_on_none_response(self):
        loop, _ = _make_loop()
    ...
    async def test_custom_session_key(self):
        loop, _ = _make_loop()
        loop.provider.chat = AsyncMock(return_value=_make_response(content="ok"))
        result = await loop.process_direct("hello", session_key="custom:key", channel="api", chat_id="req1")
        assert result == "ok"
...
class TestRunStop:
    ...
    async def test_run_starts_and_can_be_stopped(self):
        loop, bus = _make_loop()
    ...
    async def test_run_dispatches_message(self):
        loop, bus = _make_loop()
...
class TestDispatch:
    async def test_dispatch_exception_sends_error(self):
        loop, bus = _make_loop()
    ...
    async def test_dispatch_none_response_cli_publishes_empty(self):
        loop, bus = _make_loop()
    ...
    async def test_dispatch_none_response_non_cli_no_publish(self):
        loop, bus = _make_loop()
        loop._process_message = AsyncMock(return_value=None)
        msg = InboundMessage(channel="slack", sender_id="u1", chat_id="c1", content="hi")
        await loop._dispatch(msg)
